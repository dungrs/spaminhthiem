const Customer = {
    passwordFieldsHTML: `
        <div class="col-md-6 mb-3 password-section">
            ...
            <div class="position-relative password-form">
                <input value="" id="password" name="password" type="password" class="form-control password-input password" placeholder="${passwordModalConfig.placeholder}" autocomplete="off">
        ...
        <div class="col-md-6 mb-3 password-section">
            ...
            <div class="position-relative password-form">
                <input value="" id="re_password" name="re_password" type="password" class="form-control password-input re_password" placeholder="${passwordModalConfig.confirm_placeholder}" autocomplete="off">
                <button type="button" class="btn btn-link position-absolute end-0 top-0 password-addon">
                    <i class="mdi mdi-eye-outline font-size-18 text-muted"></i>
                </button>
            </div>
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    Customer.sendDataFilter(HT.currentPage);
                } else {
        ...
        $('.edit-button-modal').on('click', function (e) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    let customerCatalogueSelect = $('select[name="customer_catalogue_id"]'),
                        provinceSelect = $('select[name="province_id"]'),
                        districtSelect = $('select[name="district_id"]'),
                        wardSelect = $('select[name="ward_id"]');

                    HT.setValueSwitchChoices(customerCatalogueSelect, item.customer_catalogue_id);
    ...
    bindStoreAndUpdateEntityHandler: function () {
        $(document).on('click', '.submitButton', function (e) {
            ...
            let url = id ? `/ajax/customer/update/${id}` : '/ajax/customer/create';
            Customer.submitFormData(url);
        });
    },
    ...
    sendDataFilter: function (page = 1) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.data.data.forEach(item => {
                    tbody.append(`
                        <tr>
                            ...
                            <td>${item.address ?? '-'}</td>
                            <td>${item.customer_catalogue_name ?? '-'}</td>
                            <td>
                ...
                HT.handleDeleteRequest(".delete-btn", "/ajax/customer/delete", Customer)
    ...
    togglePasswordForm: function() {
        ...
        if(formMode === 'edit') {
            ...
            $('.birthday').closest('.col-md-6').after(Customer.passwordFieldsHTML)
        }
    }
};
...
$(document).ready(function () {
    ...
    Customer.bindStoreAndUpdateEntityHandler();
    Customer.sendDataFilter();
    Customer.setupDatetimePickerBasic();
    Customer.attachFilterEvent();
    Customer.attachPaginationEvent();
    Customer.togglePasswordForm()
});