function egaFlashSale() {
	...
	function renderCountDown(distance) {
		...
		let hours = `<div><div class="ega-badge-ctd__item ega-badge-ctd__h">${getHours(distance) >= 10 ? getHours(distance) : `0${getHours(distance)}`}</div><span>Giá»</span> </div>`
		let minutes = `<div><div class="ega-badge-ctd__item  ega-badge-ctd__m">${getMinutes(distance) >= 10 ? getMinutes(distance) : `0${getMinutes(distance)}`}</div><span>PhĂºt</span></div>`
		let seconds = `<div><div class="ega-badge-ctd__item ega-badge-ctd__s">${getSeconds(distance) >= 10 ? getSeconds(distance) : `0${getSeconds(distance)}`}</div><span>GiĂ¢y</span></div>`
		html = [hours, minutes, seconds]
		if (days > 0) html.unshift(`<div><div class="ega-badge-ctd__item ega-badge-ctd__d">${days >= 10 ? days : `0${days}`}</div><span>NgĂ y</span></div>`)
