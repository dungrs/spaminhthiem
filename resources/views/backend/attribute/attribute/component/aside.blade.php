<div class="card shadow-sm">
    <a href="#addattribute-parent-collapse" class="text-dark" data-bs-toggle="collapse" aria-expanded="true">
        <div class="p-4">
    ...
    <div id="addattribute-parent-collapse" class="collapse show">
        <div class="p-4 border-top">
            <div class="mb-3 parent-select">
                <div class="attribute_catalogue_id">
                    <label class="form-label fw-semibold">{{ __('messages.parent_category') }}  <i class="uil uil-exclamation-circle text-danger"></i></label>
                    <div class="">
                        <select name="attribute_catalogue_id" class="form-control rounded choice-single">
                            @foreach ($dropdown as $key => $val)
                                <option {{ $key == old('attribute_catalogue_id', isset($model->attribute_catalogue_id) ? $model->attribute_catalogue_id : '') ? 'selected' : '' }} value="{{ $key }}">{{ $val }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            @php
                $catalogues = isset($model) ? $attribute->attribute_catalogues->pluck('id')->toArray() : [];
            ...
            <div class="mb-3">
                ...
                <select class="form-control choice-multi" name="catalogue[]" multiple>
                    @foreach ($dropdown as $key => $val)
                        <option value="{{ $key }}" 
                            {{ in_array($key, $selectedCatalogues) && (!isset($model->attribute_catalogue_id) || $key !== $model->attribute_catalogue_id) ? 'selected' : '' }}>
                            {{ $val }}
                        </option>
                    @endforeach
                </select>
...
<div class="card shadow-sm">
    <a href="#addattribute-image-collapse" class="text-dark" data-bs-toggle="collapse" aria-expanded="true">
        <div class="p-4">
            <div class="d-flex align-items-center">
                <div class="flex-shrink-0 me-3">
                    <div class="avatar">
    ...
    <div id="addattribute-image-collapse" class="collapse show">
        <div class="p-4 border-top">
