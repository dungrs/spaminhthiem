const ProductCatalogue = {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    localStorage.setItem("productCatalogueMessage", response.message);
                    window.location.href = "/product/catalogue/index";
                } else {
    ...
    bindStoreAndUpdateEntityHandler: function () {
        $(document).on('click', '.submitButton', function (e) {
            ...
            let url = id ? `/ajax/product/catalogue/update/${id}/${language_id}` : '/ajax/product/catalogue/create';
            ProductCatalogue.submitFormData(url);
        });
    },

    sendDataFilter: function (page = 1) {
        if (!window.location.pathname.includes('/product/catalogue/index')) {
            return;
        }
        ...
        $.ajax({
            url: '/ajax/product/catalogue/filter',
            type: 'GET',
            data: dataFilterSend,
            dataType: 'json',
            success: function (response) {
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a href="${Config.baseUrl}/product/catalogue/edit/${item.id}/${item.language_id}" class="dropdown-item edit-button-modal">
                ...
                HT.openChangeStatus();
                HT.handleDeleteRequest(".delete-btn", "/ajax/product/catalogue/delete", ProductCatalogue);
...
$(document).ready(function () {
    ProductCatalogue.bindStoreAndUpdateEntityHandler();
    ProductCatalogue.sendDataFilter();
    ProductCatalogue.attachFilterEvent();
    ProductCatalogue.attachPaginationEvent();
    HT.showStoredMessage("productCatalogueMessage", "/product/catalogue/index");
