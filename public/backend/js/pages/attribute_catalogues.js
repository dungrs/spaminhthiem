const AttributeCatalogue = {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    localStorage.setItem("attributeCatalogueMessage", response.message);
                    window.location.href = "/attribute/catalogue/index";
                } else {
    ...
    bindStoreAndUpdateEntityHandler: function () {
        $(document).on('click', '.submitButton', function (e) {
            ...
            let url = id ? `/ajax/attribute/catalogue/update/${id}/${language_id}` : '/ajax/attribute/catalogue/create';
            AttributeCatalogue.submitFormData(url);
        });
    },

    sendDataFilter: function (page = 1) {
        if (!window.location.pathname.includes('/attribute/catalogue/index')) {
            return;
        }
        ...
        $.ajax({
            url: '/ajax/attribute/catalogue/filter',
            type: 'GET',
            data: dataFilterSend,
            dataType: 'json',
            success: function (response) {
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a href="${Config.baseUrl}/attribute/catalogue/edit/${item.id}/${item.language_id}" class="dropdown-item edit-button-modal">
                ...
                HT.openChangeStatus();
                HT.handleDeleteRequest(".delete-btn", "/ajax/attribute/catalogue/delete", AttributeCatalogue);
    ...
    attachPaginationEvent: function () {
        $(document).on('click', '.pagination .page-link', function (e) {
            ...
            AttributeCatalogue.sendDataFilter(HT.currentPage);
        });
    },
    ...
    attachFilterEvent: function () {
        $(".filter-data").find("input, select").on("input change", function () {
            ...
            HT.filterTimeout = setTimeout(() => {
                AttributeCatalogue.sendDataFilter();
            }, 500);
        });
    },
};
...
$(document).ready(function () {
    AttributeCatalogue.bindStoreAndUpdateEntityHandler();
    AttributeCatalogue.sendDataFilter();
    AttributeCatalogue.attachFilterEvent();
    AttributeCatalogue.attachPaginationEvent();
    HT.showStoredMessage("attributeCatalogueMessage", "/attribute/catalogue/index");
    HT.attachFilterEvent(AttributeCatalogue);
});