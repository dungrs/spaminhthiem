const PostCatalogue = {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    localStorage.setItem("postCatalogueMessage", response.message);
                    window.location.href = "/post/catalogue/index";
                } else {
    ...
    bindStoreAndUpdateEntityHandler: function () {
        $(document).on('click', '.submitButton', function (e) {
            ...
            let url = id ? `/ajax/post/catalogue/update/${id}/${language_id}` : '/ajax/post/catalogue/create';
            PostCatalogue.submitFormData(url);
        });
    },

    sendDataFilter: function (page = 1) {
        if (!window.location.pathname.includes('/post/catalogue/index')) {
            return;
        }
        ...
        $.ajax({
            url: '/ajax/post/catalogue/filter',
            type: 'GET',
            data: dataFilterSend,
            dataType: 'json',
            success: function (response) {
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a href="${Config.baseUrl}/post/catalogue/edit/${item.id}/${item.language_id}" class="dropdown-item edit-button-modal">
                ...
                HT.openChangeStatus();
                HT.handleDeleteRequest(".delete-btn", "/ajax/post/catalogue/delete", PostCatalogue);
    ...
    attachPaginationEvent: function () {
        $(document).on('click', '.pagination .page-link', function (e) {
            ...
            PostCatalogue.sendDataFilter(HT.currentPage);
        });
    },
...
$(document).ready(function () {
    PostCatalogue.bindStoreAndUpdateEntityHandler();
    PostCatalogue.sendDataFilter();
    PostCatalogue.attachFilterEvent();
    PostCatalogue.attachPaginationEvent();
    HT.showStoredMessage("postCatalogueMessage", "/post/catalogue/index");
    HT.attachFilterEvent(PostCatalogue);
});