        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    modal.modal('hide');
                    $('#' + formId)[0].reset();
                    Source.sendDataFilter(HT.currentPage);
                } else {
    ...
    bindSubmitEntityHandler: function () {
        $(document).on('click', '.submitButton', function (e) {
            ...
            if (modal.hasClass('store-modal')) {
                ...
                url = id ? `/ajax/source/update/${id}` : '/ajax/source/create/';
            }
    
            Source.submitFormData(url, formId, formData);
        });
    },
    ...
    sendDataFilter: function (page = 1) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                HT.openChangeStatus();
                Source.openEditModal();
                HT.handleDeleteRequest(".delete-btn", "/ajax/source/delete", Source)
...
$(document).ready(function () {
    Source.bindSubmitEntityHandler();
    Source.sendDataFilter();
    Source.openAddModal();
    HT.attachFilterEvent(Source);
    Source.attachPaginationEvent();
});